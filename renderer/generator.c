...
static uint32_t run_block(SGS_Generator *restrict o,
	...
	if (fmodc) {
		float *freq2 = *(bufs++);
		SGS_Ramp_run(&n->freq2, freq2, len, o->srate,
		...
		SGS_Ramp_skip(&n->freq2, len, o->srate, &n->freq2_pos);
	}
	/*
	...
	if (amodc) {
		float *amp2 = *(bufs++);
		SGS_Ramp_run(&n->amp2, amp2, len, o->srate, &n->amp2_pos, NULL);
		...
		SGS_Ramp_skip(&n->amp2, len, o->srate, &n->amp2_pos);
	}
	if (!wave_env) {
		const float *lut = SGS_Wave_luts[n->wave];
		SGS_Osc_block_add(&n->osc, lut, o->osc_coeff,
				s_buf, len, acc_ind, freq, amp, pm_buf);
	} else {
		const float *lut = SGS_Wave_luts[n->wave];
		SGS_Osc_block_mul(&n->osc, lut, o->osc_coeff,
				s_buf, len, acc_ind, freq, amp, pm_buf);
	}
	/*
